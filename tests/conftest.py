...
def clone_test_database():
    ...
    # Apply migration 005 (Drop Part table)
...
MIGRATION_005_SQL = os.path.join(
    MIGRATIONS_DIR, '005_drop_part.sql'
)
...
def _apply_migration_003():
    """Apply migration 003 SQL to the test database."""
    if not os.path.exists(MIGRATION_003_SQL):
        print("[setup] Migration 003 SQL not found, skipping")
        return
    print("[setup] Applying migration 003...")
    result = subprocess.run(
        ['psql', '-U', DB_USER, '-d', TEST_DB_NAME,
         '-f', MIGRATION_003_SQL],
        capture_output=True, text=True
...
def _apply_migration_005():
    """Apply migration 005 SQL to the test database."""
    if not os.path.exists(MIGRATION_005_SQL):
        print("[setup] Migration 005 SQL not found, skipping")
        return
    print("[setup] Applying migration 005...")
    result = subprocess.run(
        ['psql', '-U', DB_USER, '-d', TEST_DB_NAME,
         '-f', MIGRATION_005_SQL],
        capture_output=True, text=True
    ...
    if result.returncode != 0:
        raise RuntimeError(
            f"Migration 005 failed: {result.stderr[:400]}"
        )
    print("[setup] Migration 005 applied")


def _apply_migration_006():
    """Apply migration 006 SQL to the test database."""
    if not os.path.exists(MIGRATION_006_SQL):
        print("[setup] Migration 006 SQL not found, skipping")
        return
    print("[setup] Applying migration 006...")
    result = subprocess.run(
        ['psql', '-U', DB_USER, '-d', TEST_DB_NAME,
         '-f', MIGRATION_006_SQL],
        capture_output=True, text=True
...
def _apply_migration_009():
    """Apply migration 009 SQL to the test database."""
    if not os.path.exists(MIGRATION_009_SQL):
        print("[setup] Migration 009 SQL not found, skipping")
        return
    print("[setup] Applying migration 009...")
    result = subprocess.run(
        ['psql', '-U', DB_USER, '-d', TEST_DB_NAME,
         '-f', MIGRATION_009_SQL],
        capture_output=True, text=True
...
def _apply_migration_011():
    """Apply migration 011 SQL to the test database."""
    if not os.path.exists(MIGRATION_011_SQL):
        print("[setup] Migration 011 SQL not found, skipping")
        return
    print("[setup] Applying migration 011...")
    result = subprocess.run(
        ['psql', '-U', DB_USER, '-d', TEST_DB_NAME,
         '-f', MIGRATION_011_SQL],
        capture_output=True, text=True
